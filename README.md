...
## 기술 선택 근거: RAG 방식 정량적 비교 실험

규정 질의응답 기능에 RAG(검색 증강 생성)를 채택할 근거를 남기기 위해, 회의실 예약 규정 문서를 기반으로 만든 질문-정답 세트 15개에 대해 3가지 방식으로 답변을 생성해 비교 실험했다.

**실험 방법**: 동일한 15개 질문에 대해 다음 세 가지 방식으로 각각 답변을 생성하고, 정확도와 평균 토큰 사용량을 측정했다.
1. No RAG — 문서 없이 질문만 전달 (모델의 사전 지식에만 의존)
2. 전체 문서 주입 — 검색 없이 규정 문서 전체를 매번 프롬프트에 포함
3. RAG — pgvector로 관련 청크만 검색해 주입 (실제 구현 방식)

**결과**

| 방식 | 정확도 | 평균 토큰(질문당) |
|---|---|---|
| No RAG | 6.7% (1/15) | 295.3 |
| 전체 문서 주입 | 93.3% (14/15) | 450.3 |
| RAG | 100% (15/15) | 256.5 |

RAG는 관련 청크만 검색해 주입함으로써 정확도와 비용을 동시에 확보했다. No RAG는 문서를 참조하지 않아 대부분 할루시네이션이 발생했고, 전체 문서 주입은 정확도는 준수하지만 매번 전체 문서를 보내 토큰 비용이 RAG보다 더 높았다. 문서가 길어질수록 전체 문서 주입 대비 RAG의 정확도·비용 격차는 더 벌어질 것으로 예상된다.
